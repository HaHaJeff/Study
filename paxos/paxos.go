...
// My code
// in the instance, if Acceptor's proposal id <
func (px *Paxos) Accept(args *AcceptArgs, reply *AcceptReply) error {
	...
	if reply.Err == OK {
		if PrintDebug {
			fmt.Printf("%s:%d accept %v", px.peers[px.me], args.Seq, args.Value)
		}
...
func (px *Paxos) generatePNum() string {
	begin := time.Date(2018, time.May, 26, 0, 0, 0, 0, time.UTC)
...
func (px *Paxos) sendPrepare(seq int, v interface{}) (bool, string, interface{}) {
	...
	if PrintDebug {
		fmt.Printf("%s send prepare %s:%v\n", px.peers[px.me], pnum, v)
	}
	...
	for i, peer := range px.peers {
		...
		if reply.Err == OK {
			...
			// update accept num and value if reply accept num >
	...
	// return reply value
	// why return pnum not replyPNum ?
	// cause use pnum to propose replyValue
